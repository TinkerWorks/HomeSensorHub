...
class TSL258x:
    ...
    def read(self):
        (data0low, ) = self.bus.read_i2c_block_data(
            self.addr, self.COMMAND_CMD | self.TRANSACTION | self.DATA0LOW, 1)
        (data0high, ) = self.bus.read_i2c_block_data(
            self.addr, self.COMMAND_CMD | self.TRANSACTION | self.DATA0HIGH, 1)
        (data1low, ) = self.bus.read_i2c_block_data(
            self.addr, self.COMMAND_CMD | self.TRANSACTION | self.DATA1LOW, 1)
        (data1high, ) = self.bus.read_i2c_block_data(
            self.addr, self.COMMAND_CMD | self.TRANSACTION | self.DATA1HIGH, 1)
    ...
    def power_on(self):
        self.bus.write_byte_data(
            self.COMMAND_CMD | self.CONTROL, self.CONTROL_POWERON)
    ...
    def config(self, gain=16, integration_cycles=NOMINAL_INTEGRATION_CYCLES):
        ...
        self.bus.write_byte_data(
            self.addr, self.COMMAND_CMD | self.TIMING, timing_value)
        self.bus.write_byte_data(
            self.addr,
            self.COMMAND_CMD | self.CONTROL, self.ADC_EN | self.CONTROL_POWERON)
        # Every ADC cycle generates interrupt
        self.bus.write_byte_data(
            self.addr, self.COMMAND_CMD | self.INTERRUPT, self.INTR_INTER_MODE)
        # GAIN = 16
    ...
    def calculateLux(self, ch0, ch1):
        ...
        if (self.integration_cycles == self.NOMINAL_INTEGRATION_CYCLES):
            ...
            chScale0 = (self.NOMINAL_INTEGRATION_CYCLES << self.CH_SCALE) / self.tIntCycles
        ...
        if (self.gain == 1):  # 1x gain
            ...
            chScale1 = chScale0 / self.CH1GAIN128X
            chScale0 = chScale0 / self.CH0GAIN128X
